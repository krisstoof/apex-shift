...
namespace ApexShift.Presentation.HUD
{
    public sealed class ActionBarView : MonoBehaviour
    {
        ...
        private void Build()
        {
            ...
            int targetSlotCount = actionBar != null && actionBar.State != null ? actionBar.State.SlotCount : 0;
            while (slotsRoot.childCount > targetSlotCount)
            {
                GameObject surplus = slotsRoot.GetChild(slotsRoot.childCount - 1).gameObject;
                DestroyImmediate(surplus);
            }
            while (slotsRoot.childCount < targetSlotCount)
            {
